...
func TestCmapNew(t *testing.T) {
	var concurrency int
	var pr PairRedistributor

	// case1:
	cm, err := NewConcurrentMap(concurrency, pr)
	if err == nil {
	...
	cm, err = NewConcurrentMap(concurrency, pr)
	if err != nil {
		t.Fatalf("Error: new concurrent map : %s (concurrency: %d, pairRedistributor: %#v)",
			err, concurrency, pr,
		)
	}

	if cm == nil {
		t.Fatalf("Error: Couldn't new a concurrent map! (concurrency: %d, pairRedistributor: %#v",
			concurrency, pr,
		)
	}

	if cm.Concurrency() != concurrency {
		t.Fatalf("Error: inconsistent concurrency: expected: %d, actural: %d",
