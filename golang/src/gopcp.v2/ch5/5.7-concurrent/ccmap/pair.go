...
func (p *aPair) Key() string {
	return p.key
}

//
func (p *aPair) Hash() uint64 {
	return p.hash
}

//
func (p *aPair) Element() interface{} {
...
//
func (p *aPair) SetElement(element interface{}) error {
	if element == nil {
...
func (p *aPair) Next() Pair {
	...
	return (*aPair)(pointer)
}

//
func (p *aPair) SetNext(nextPair Pair) error {
	if nextPair == nil {
...
//
func (p *aPair) Copy() Pair {
	pCopy, _ := newPair(p.Key(), p.Element())
	return pCopy
}

//
func (p *aPair) String() string {
	return p.genString(false)
}

// 生成 key-value 对的字符串表示形式
func (p *aPair) genString(nextDetail bool) string {
